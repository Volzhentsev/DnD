...
function saveToStorage() {
  var taskBoardElement = document.body.querySelector('.board-wrapper');
  var taskBoardHtml = taskBoardElement.innerHTML;
...
function itemDrag(event) {
  ...
  draggedItem.style.left = "".concat(event.pageX - shiftX, "px");
  draggedItem.style.top = "".concat(event.pageY - shiftY, "px");
  draggedItem.style.visibility = 'hidden';
  var closestTarget = document.elementFromPoint(event.clientX, event.clientY);
  draggedItem.style.visibility = '';
  if (!closestTarget) return;
  var targetDrop = closestTarget.closest('.items-task');
  var targetDropPanel = closestTarget.closest('.panel');
  if (!targetDropPanel) return;
  var targetDropPanelList = targetDropPanel.querySelector('.items');
  ...
  var _targetDrop$getBoundi = targetDrop.getBoundingClientRect(),
    top = _targetDrop$getBoundi.top;
...
function setDragAndDropHandlers(panelItemsListLastChild) {
  panelItemsListLastChild.addEventListener('mousedown', function (event) {
    ...
    var _currentItem$getBound = currentItem.getBoundingClientRect(),
      left = _currentItem$getBound.left,
      top = _currentItem$getBound.top;
...
function setItemRemoveHandlers(item) {
  ...
  item.querySelector('.item-remove').addEventListener('click', function (event) {
...
function setAddTaskButtonHandlers(addTasksItem, addTasksForm, newTaskTitle) {
  addTasksItem.addEventListener('click', function () {
...
function setCancelAddTaskButtonHandlers(addTasksItem, addTasksForm, cancelTasksBtn) {
  cancelTasksBtn.addEventListener('click', function () {
...
function setAddTaskSubmitHandlers(addTasksItem, addTasksForm, panelItemsList, newTaskTitle) {
  addTasksForm.addEventListener('submit', function (event) {
    event.preventDefault();
    var inputText = newTaskTitle.value.trim();
    if (inputText === '') return;
...
document.addEventListener('DOMContentLoaded', function () {
  var loadedState = loadFromStorage();
  if (loadedState) {
    var taskBoardElement = document.body.querySelector('.board-wrapper');
    taskBoardElement.innerHTML = loadedState;
    var taskItems = taskBoardElement.querySelectorAll('.items-task');
    taskItems.forEach(function (item) {
      setItemRemoveHandlers(item);
      setDragAndDropHandlers(item);
    });
  }
  var board = document.body.querySelector('.board-wrapper');
  var panels = board.querySelectorAll('.panel');
  panels.forEach(function (panel) {
    var addTasksContainer = panel.querySelector('.add-tasks-container');
    var panelItemsList = panel.querySelector('.items');
    var addTasksForm = addTasksContainer.querySelector('.add-tasks-control');
    var newTaskTitle = addTasksForm.querySelector('.add-tasks-input');
    var cancelTasksBtn = addTasksForm.querySelector('.add-tasks-cancel');
    var addTasksItem = panel.querySelector('.add-tasks-item');
